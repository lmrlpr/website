...
const DigitalSerenity = () => {
  ...
  useEffect(() => {
    const wordElements = document.querySelectorAll('.word-animate');
    const handleMouseEnter = (e: Event) => { (e.target as HTMLElement).style.textShadow = '0 0 20px rgba(203, 213, 225, 0.5)'; };
    const handleMouseLeave = (e: Event) => { (e.target as HTMLElement).style.textShadow = 'none'; };
    wordElements.forEach(word => {
    ...
    const handleScroll = () => {
      if (!scrolled) {
        ...
        floatingElementsRef.current.forEach((el, index) => {
          ...
          }, (parseFloat(htmlEl.style.animationDelay || '0') * 1000) + index * 100);
        });
      }
    };
